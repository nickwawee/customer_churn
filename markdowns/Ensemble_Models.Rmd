...
bank <- bank[,c(-1,-2,-5,-7:-10, -39:-42)]#removes card type
...
rf <- train(Attrition_Flag ~ ., data = training.data, method = "rf", ntree = 50,rControl = train_control, tuneGrid = expand.grid(mtry = c(4, 5, 6, 7, 8)), metric = 'Kappa')
...
write.csv(x = prob.test, file = '../data/processed/rf_prob.csv')
...
prob.test <- predict(ada, test.data, type = 'prob')
write.csv(x = prob.test, file = '../data/processed/ada_prob.csv')
# create confusion matrix

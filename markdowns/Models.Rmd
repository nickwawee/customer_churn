...
```{r}
# Scaling the data
training.scaled <- scale(training.data[,c(-1,-3,-5:-8)], center = TRUE, scale = TRUE)
attrib <- attributes(training.scaled)
test.scaled <- scale(test.data[,c(-1,-3,-5:-8)], center = attrib$`scaled:center`, scale = attrib$`scaled:scale`)

library(FNN)
# Fitting 10-fold CV model
set.seed(0)
train_control <- trainControl(method = "cv", number = 10) 
knn_cv <- train(Attrition_Flag ~ ., data = training.data, method = "knn", trControl = train_control, tuneLength = 10, metric = "Kappa")

# Final model
print(knn_cv)
knn_cv$finalModel

# Fit model on test data with k = 15
knn.test <- knn(train = training.scaled, test = test.scaled, cl = training.data[,1], k = 15)
